		...
		function($compile, $timeout, taOptions, taSelection, taExecCommand, textAngularManager, $window, $document, $animate, $log){
			return {
				...
				link: function(scope, element, attrs, ngModel){
					...
					_originalContents = element[0].innerHTML;0
					...
					// console.log('------------Toolbar load --', _toolbars);
			...
			return {
				...
				link: function(scope, element, attrs){
					...
					var setupToolElement = function(toolDefinition, toolScope){
						...
						if (toolDefinition.buttontext == 'A') {
							toolElement.addClass('toggle-btn');
						}
					...
					var defaultChildScope = {
						...
						isDisabled: function(){
							...
							return ( // this bracket is important as without it it just returns the first bracket and ignores the rest
								...
								(this.name !== 'html' && this.name !== 'toggle' && this.$editor().showHtml) ||
					...
					angular.forEach(scope.toolbar, function(group, idx){
						// setup the toolbar group
						var groupElement = angular.element("<div>");
						groupElement.addClass(scope.classes.toolbarGroup);
						if (idx !== 0) {
							groupElement.attr('ng-hide', 'isToggle');
							groupElement.addClass('toggle-options');
							groupElement.attr('style', 'display: none;');
						}
